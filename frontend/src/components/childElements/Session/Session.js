    ...
    return (
        <div className="Session">
            <Link to={`/nodes/${props.node.node_uuid}`} className="sessionHeader">
                ...
                <div className="time">
                    <span className="startTime">{moment.unix(props.node.session_start).format("h:mm A")}</span> - <span className="endTime">{moment.unix(props.node.session_end).format("h:mm A")}</span>
                </div>
