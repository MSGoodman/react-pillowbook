...
function Attachment(props) {
    function updateHorizontal() {
        ...
        updateNode(updatedNode).then((updated) =>
            props.setNewestNodeUpdate(
                "Set new horizontal image: " + props.node.name
            )
        );
    }
    function updateVertical() {
        ...
        updateNode(updatedNode).then((updated) =>
            props.setNewestNodeUpdate(
                "Set new vertical image: " + props.node.name
            )
        );
    }

    const imageExtensions = ["jpg", "png", "svg", "jpeg", "gif", "jfif"];
    const src = imageExtensions.includes(
        props.node.file_extension.toLowerCase()
    )`/static/media/${props.node.node_uuid + "." + props.node.file_extension}`;
    const image = imageExtensions.includes(
        props.node.file_extension.toLowerCase()
    ) ? (
        <img src={src} alt={props.node.name + " image"} />
    ) : (
        <img
            className="nonImageImage"
            src={require("../../../assets/images/file-archive.svg")}
            alt="generic attachment"
        />
    );
    ...
    return (
        <div className="Attachment">
            ...
            <a title="Download file" href={src} download>
                <i className="fas fa-download"></i>
            </a>
            <button title="Set as horizontal image" onClick={updateHorizontal}>
                <i className="fas fa-arrows-alt-h"></i>
            </button>
            <button title="Set as vertical image" onClick={updateVertical}>
                <i className="fas fa-arrows-alt-v"></i>
            </button>
            <Link
                to={`/nodes/${props.node.node_uuid}`}
                title="Navigate to file page"
            >
                {" "}
